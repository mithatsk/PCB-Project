...
public class ToolBar extends JPanel
{
   ...
   public ToolBar(Graph graph)
   {
      ...
      JToggleButton grabberButton = new JToggleButton(new Icon()
      {
         ...
         public void paintIcon(Component c, Graphics g, int x, int y)
         {
            ...
            GraphPanel.drawGrabber2(g2, x + OFFSET, y + OFFSET);
            GraphPanel.drawGrabber2(g2, x + OFFSET, y + BUTTON_SIZE - OFFSET);
            GraphPanel.drawGrabber2(g2, x + BUTTON_SIZE - OFFSET, y + OFFSET);
            GraphPanel.drawGrabber2(g2, x + BUTTON_SIZE - OFFSET, y + BUTTON_SIZE - OFFSET);
   ...
   public Object getSelectedTool()
   {
      ...
      for (Object o : tools)
      {
         ...
         if (button.isSelected()) {
            //System.out.println("this is selected at the toggle object: " + o);
            return o;
         }
      }
      return null;
   ...
   public void add(final Node n)
   {
      ...
      JToggleButton button = new JToggleButton(new Icon() {
         public int getIconHeight() { return BUTTON_SIZE; }
         public int getIconWidth() { return BUTTON_SIZE; }
         public void paintIcon(Component c, Graphics g, int x, int y) {//kebab
            System.out.println("Selected buttons in Tool class: "+n.getClass().getName().toString());
            if(n.getClass().getName().toString()== "resistorNode"){
               double width = n.getBounds().getWidth()/1.3;
               double height = n.getBounds().getHeight()/1.3;
               Graphics2D g2 = (Graphics2D) g;
               double scaleX = (BUTTON_SIZE - OFFSET)/ width;
               double scaleY = (BUTTON_SIZE - OFFSET)/ height/1.6;
               double scale = Math.min(scaleX, scaleY);

               AffineTransform oldTransform = g2.getTransform();
               g2.translate(x, y+10);
               g2.scale(scale, scale);
               g2.translate(Math.max((height - width) / 2-20, -20), Math.max((width - height) / 2, 0));
               g2.setColor(Color.black);
               n.draw(g2);

               g2.setTransform(oldTransform);

            }else if(n.getClass().getName().toString()=="voltageSupplyNode"){
               double width = n.getBounds().getWidth()*1.5;
               double height = n.getBounds().getHeight()/2;
               Graphics2D g2 = (Graphics2D) g;
               double scaleX = (BUTTON_SIZE - OFFSET)/ width;
               double scaleY = (BUTTON_SIZE - OFFSET)/ height/1.6;
               double scale = Math.min(scaleX, scaleY);
